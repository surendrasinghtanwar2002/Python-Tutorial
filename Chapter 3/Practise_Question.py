...
##Approach 6
#Creating a tuple 
my_tuple = ("apple",10,"banana",5,"cherry",20)

#creating a set variable to store the iteration value 
my_set = set()
my_list = []

#Iterating values and storing the value into the set for removing duplicate value
for item in my_tuple:
    if item not in my_set:
        my_set.add(item)
        my_list.append(item)

#Sorting string and integer and merging them into a single list after succesful sorting 
string_sorting = sorted([x for x in my_list if isinstance(x, str)])
numerical_sorting = sorted([j for j in my_list if isinstance(j,int)])
new_list = string_sorting+numerical_sorting
print(new_list)
